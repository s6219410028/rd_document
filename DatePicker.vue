<!--
  DatePicker.vue — zero-dependency, portable Vue 3 date picker
  ─────────────────────────────────────────────────────────────
  Props:
    v-model          String   'YYYY-MM-DD'  (emits same format)
    placeholder      String   default 'dd/mm/yyyy'
    min              String   'YYYY-MM-DD'  — earliest selectable date
    max              String   'YYYY-MM-DD'  — latest selectable date
    disabled         Boolean  disable the whole picker
    firstDayOfWeek   Number   0 = Sunday (default), 1 = Monday

  Usage:
    <DatePicker v-model="form.date" />
    <DatePicker v-model="form.expire" min="2024-01-01" placeholder="Select expiry" />

  Emits:
    update:modelValue  String 'YYYY-MM-DD' | ''
-->
  ...
  <div class="dp-wrap" ref="wrapEl">
    ...
    <Transition name="dp-pop">
      <div v-if="isOpen" class="dp-popup" @click.stop>
        ...
        <!-- ── Year picker ────────────────────────────────────── -->
        <div v-if="mode === 'year'" class="dp-year-list" ref="yearListEl">
          <button
...
const props = defineProps({
  ...
  firstDayOfWeek: { type: Number,  default: 0 },   // 0 = Sun, 1 = Mon
})
...
// today at midnight — used for "today" highlight
...
/* ── Sync viewport to modelValue when it changes externally ──── */
...
const cells = computed(() => {
  ...
  // offset = how many cells before the 1st of this month
...
<style scoped>
/* ── CSS variables with fallbacks (works standalone) ─────────── */
.dp-wrap {
  --dp-accent:   var(--c-teal, #00e5a0);
  --dp-surface:  var(--surface, #fff);
  --dp-border:   var(--border, #e2e6ed);
  --dp-text:     var(--text,   #111827);
  --dp-muted:    var(--c-muted,  #718096);
  --dp-bg:       var(--bg,     #f4f6f9);
  --dp-font:     var(--font, 'Noto Sans Thai', system-ui, sans-serif);
  --dp-r:        8px;
  --dp-shadow:   0 8px 24px rgba(0,0,0,.12);
  position: relative;
...
.dp-field--open                { border-color: var(--dp-accent); box-shadow: 0 0 0 3px color-mix(in srgb, var(--dp-accent) 20%, transparent); }
.dp-field--disabled            { opacity: .5; cursor: not-allowed; pointer-events: none; }
.dp-cal-icon  { width: 15px; height: 15px; color: var(--dp-muted); flex-shrink: 0; }
.dp-display   { flex: 1; font-size: 14px; color: var(--dp-text); }
...
/* popup animation */
.dp-pop-enter-active, .dp-pop-leave-active { transition: opacity .15s, transform .15s; }
.dp-pop-enter-from, .dp-pop-leave-to       { opacity: 0; transform: translateY(-6px); }
...
.dp-hdr-btn:hover { background: var(--dp-bg); border-color: #1e2340; }
...
.dp-day--other                     { color: var(--dp-muted); opacity: .45; }
...
.dp-day--disabled                  { color: #ccc; cursor: not-allowed; opacity: .4; }
...
.dp-today-btn { background: rgba(0,229,160,.12); color: var(--dp-accent); border-color: rgba(0,229,160,.3); }
.dp-today-btn:hover { background: rgba(0,229,160,.22); }
.dp-clear-btn { background: none; color: var(--dp-muted); border-color: #1e2340; }
